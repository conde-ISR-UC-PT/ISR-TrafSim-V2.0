...
function occ=check_cross_entrance_isr_trafsim(roadID)
global s
...
cornX(3)=cornX(3)-s.amplitudeCrossControl(1)*cos(t); cornY(3)=cornY(3)-s.amplitudeCrossControl(1)*sin(t);
cornX(4)=cornX(4)-s.amplitudeCrossControl(1)*cos(t); cornY(4)=cornY(4)-s.amplitudeCrossControl(1)*sin(t);
...
if(isempty(resMask) || length(resMask)==1)   % If not exist obstacles in selected space
    s.intersectCarCross(:,roadID)=Inf;
    if(s.cross_mode==1 || s.cross_mode==2)
        s.lights(2,roadID)=0;
    end
else
